...
public class VerCarroAdapter extends RecyclerView.Adapter<VerCarroAdapter.ViewHolder> {
    ...
    public void calcularTotal(int cant){
        double total=0;
        if(carroCompra != null){
            for(int i = 0 ; i < carroCompra.size() ; i++) {
                total = total + Double.parseDouble(""+carroCompra.get(i).getPrecio() * cant);
            }
    ...
    public void onBindViewHolder(ViewHolder holder, int position) {
        ...
        holder.tvPrecio.setText(""+carroCompra.get(position).getPrecio() + "€");
        Picasso.get().load(carroCompra.get(position).getImagen()).into(holder.imagenRestaurante);
        int cant = carroCompra.get(position).getCantidad();
        ...
        holder.btnEliminarPCarrito.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                //calcularTotal(cant,holder.getAdapterPosition());
                //calcularTotalOnly();
